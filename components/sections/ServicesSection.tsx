...
export function ServicesSection() {
  ...
  // Merge translated titles/descriptions with config images/slugs
  const services = siteConfig.services.map((s, i) => ({
    ...s,
    title: t.services.items[i]?.title ?? s.title,
    shortDescription: t.services.items[i]?.shortDescription ?? s.shortDescription,
  }));
          ...
          {services.map((service, index) => (
            ...
            >
              ...
              <div className="w-full md:w-1/2">
                <span className="text-brand-wood text-lg mb-4 block font-serif italic">
                  {service.id}
                </span>
                <h3 className="text-3xl md:text-4xl font-serif text-brand-charcoal mb-6">
                  {service.title}
                </h3>
